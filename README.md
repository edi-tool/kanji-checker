...
- PDF の作成方法（画像化された PDF など）によっては、テキストが抽出できない場合があります。
- PDF のフォント内部の文字マッピングにより、見た目は常用漢字でも別の文字コード（康熙部首・異体字など）として抽出され、表外字として誤検知されることがあります（対応は下記「メンテナンスフロー」）。
- 人名用漢字の判別データ（`jinmei-kanji-data.js`）は、旧字体の一部が常用漢字の字形に統合されており、現在 858 字です（告示は 863 字）。表外字の検出には影響しませんが、人名用漢字の印が付かない旧字体があります。
